...
For API/DB-sourced prices, `this.fetchTokenPrice(addresses, opts)` forwards `opts` to an injected `getTokenPrices`. With no injected source it falls back to the Berachain API, which serves current prices only — a `blockNumber`/`timestamp` query on that fallback rejects rather than returning stale current prices.

**Token discovery.** `getTokens()` returns only the protocol-specific tokens this adapter prices on-chain (LP tokens, vault shares) — not well-known tokens like WBERA, HONEY, or USDC.e, which the hub already prices. Fetch those via `fetchTokenPrice` when you need them as inputs, but don't emit them from `getTokens()`. The list can be hardcoded or discovered dynamically (e.g. from a subgraph).
...
Keep PRs focused — for new adapters, one adapter per PR makes review easier (repo-wide refactors are the exception).
