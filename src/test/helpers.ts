...
import type { GetTokenPrices, PriceQueryOptions, TokenAndPrice } from "../types";
...
export function createMockGetTokenPrices(
  ...
  return vi
    .fn()
    .mockImplementation(async (tokens: string[], opts?: PriceQueryOptions) => {
      return tokens
        .map((addr) => {
          ...
          return {
            address: addr,
            price,
            updatedAt: opts?.timestamp ?? Date.now(),
