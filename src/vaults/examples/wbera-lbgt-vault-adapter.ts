...
export class WberaLBGTVaultAdapter extends BaseAdapter {
    ...
    async getRewardVaultStakingTokenPrices(stakingTokens: Token[]): Promise<TokenPrice[]> {
        const poolData = await fetchPoolData(stakingTokens.map((token) => token.address));
        return poolData.map((pool) => ({
            address: pool.address,
            price: Number(pool.dynamicData.totalLiquidity) / Number(pool.dynamicData.totalShares),
            timestamp: Date.now(),
            chainId: 80094,
        }));
    }
