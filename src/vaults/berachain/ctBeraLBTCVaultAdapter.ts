...
export class CtBeraLBTCVaultAdapter extends BaseAdapter {
    constructor() {
        super({
            name: "CtBeraLBTCVaultAdapter",
            description:
                "Adapter for the Concrete - LBTC vault. Users receive ctBeraLBTC by depositing LBTC into Concrete’s Berachain Bitcoin vault. Vault: 0xB6E3C1154e07f8a3dc04a9a28648C7AA30511120.",
            enabled: true,
        });
    }
    ...
    async getRewardVaultStakingTokenPrices(stakingTokens: Token[]): Promise<TokenPrice[]> {
        ...
        const prices = await Promise.all(
            stakingTokens.map(async (token) => {
                ...
                if (totalSupply === 0n)
                    throw new Error(
                        `Failed to fetch LSP data for ${token.address} on 0xB6E3C1154e07f8a3dc04a9a28648C7AA30511120: totalSupply is 0`
                    );
