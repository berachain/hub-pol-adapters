...
export class CtBeraHONEYVaultAdapter extends BaseAdapter {
    constructor() {
        super({
            name: "CtBeraHONEYVaultAdapter",
            description:
                "Adapter for the Concrete - HONEY vault. Users receive ctBeraHONEY by depositing HONEY into Concrete’s Berachain Stables vault. Vault: 0x81235952ad93987F74e074994Def2a7e1D6F1Fb0.",
            enabled: true,
        });
    }
    ...
    async getRewardVaultStakingTokenPrices(stakingTokens: Token[]): Promise<TokenPrice[]> {
        ...
        const prices = await Promise.all(
            stakingTokens.map(async (token) => {
                ...
                if (totalSupply == null)
                    throw new Error(
                        `Failed to fetch LSP data for ${token.address} on 0x81235952ad93987F74e074994Def2a7e1D6F1Fb0: totalSupply is null`
                    );
