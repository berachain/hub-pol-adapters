...
export class CtBerauniBTCVaultAdapter extends BaseAdapter {
    constructor() {
        super({
            name: "CtBerauniBTCVaultAdapter",
            description:
                "Adapter for the Concrete - uniBTC vault. Users receive ctBerauniBTC by depositing uniBTC into Concrete’s Berachain Bitcoin vault. Vault: 0x068D072d1ee7647B9d649a7C2046166Aa81af3D3.",
            enabled: true,
        });
    }
    ...
    async getRewardVaultStakingTokenPrices(stakingTokens: Token[]): Promise<TokenPrice[]> {
        ...
        const prices = await Promise.all(
            stakingTokens.map(async (token) => {
                ...
                if (totalSupply == null)
                    throw new Error(
                        `Failed to fetch LSP data for ${token.address} on 0x068D072d1ee7647B9d649a7C2046166Aa81af3D3: totalSupply is null`
                    );
